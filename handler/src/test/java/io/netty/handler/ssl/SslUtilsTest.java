...
public class SslUtilsTest {
    ...
    public void testPacketLength() throws SSLException, NoSuchAlgorithmException {
        ...
        assertEquals(SslUtils.getEncryptedPacketLength(bufferBE, 0),
                SslUtils.getEncryptedPacketLength(bufferLE, 0));
        assertEquals(SslUtils.getEncryptedPacketLength(new ByteBuffer[] { bufferBE.nioBuffer() }, 0),
                SslUtils.getEncryptedPacketLength(
                        new ByteBuffer[] { bufferLE.nioBuffer().order(ByteOrder.LITTLE_ENDIAN) }, 0));
    }
